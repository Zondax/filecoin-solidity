...
library InitAPI {
    ...
    function exec(InitTypes.ExecParams memory params) internal returns (InitTypes.ExecReturn memory) {
        ...
        bytes memory raw_response = Actor.callByID(InitTypes.ActorID, InitTypes.ExecMethodNum, Misc.CBOR_CODEC, raw_request, 0, false);
    ...
    function exec4(InitTypes.Exec4Params memory params) internal returns (InitTypes.Exec4Return memory) {
        ...
        bytes memory raw_response = Actor.callByID(InitTypes.ActorID, InitTypes.Exec4MethodNum, Misc.CBOR_CODEC, raw_request, 0, false);
