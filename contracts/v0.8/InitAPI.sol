...
library InitAPI {
    ...
    function exec(InitTypes.ExecParams memory params) internal returns (InitTypes.ExecReturn memory) {
        ...
        bytes memory raw_response = Actor.callByAddress(InitTypes.ExecMethodNum, InitTypes.ActorID, raw_request, Misc.CBOR_CODEC, 0, false);
    ...
    function exec4(InitTypes.Exec4Params memory params) internal returns (InitTypes.Exec4Return memory) {
        ...
        bytes memory raw_response = Actor.callByAddress(InitTypes.Exec4MethodNum, InitTypes.ActorID, raw_request, Misc.CBOR_CODEC, 0, false);
